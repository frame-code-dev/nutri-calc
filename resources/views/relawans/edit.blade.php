<x-app-layout>
    <x-slot name="header">
        <h2 class="text-lg font-bold text-gray-800">Edit Relawan</h2>
    </x-slot>

    <div class="max-w-2xl">
        <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
            <form method="POST" action="{{ route('relawans.update', $relawan) }}" class="space-y-5">
                @csrf @method('PUT')

                <div class="grid grid-cols-1 sm:grid-cols-2 gap-5">
                    <div>
                        <label class="block text-sm font-semibold text-gray-700 mb-1">Nama Lengkap <span class="text-red-500">*</span></label>
                        <input type="text" name="nama" value="{{ old('nama', $relawan->nama) }}" required
                    ...
                    <div>
                        ...
                        <input type="text" name="jabatan" value="{{ old('jabatan', $relawan->jabatan) }}" required
                               class="w-full border border-gray-200 rounded-xl px-3 py-2 text-sm focus:ring-2 focus:ring-blue-500 focus:border-transparent">
                    </div>
                    ...
                    <div>
                        ...
                        <select name="tipe" required class="w-full border border-gray-200 rounded-xl px-3 py-2 text-sm focus:ring-2 focus:ring-blue-500 focus:border-transparent">
                            <option value="tetap" {{ old('tipe', $relawan->tipe) === 'tetap' ? 'selected' : '' }}>Relawan Tetap</option>
                            <option value="magang" {{ old('tipe', $relawan->tipe) === 'magang' ? 'selected' : '' }}>Tenaga Magang</option>
                        </select>
                    </div>

                    <div>
                        <label class="block text-sm font-semibold text-gray-700 mb-1">Nomor Urut <span class="text-red-500">*</span></label>
                        <input type="number" name="nomor_urut" value="{{ old('nomor_urut', $relawan->nomor_urut) }}" min="1" required
                               class="w-full border border-gray-200 rounded-xl px-3 py-2 text-sm focus:ring-2 focus:ring-blue-500 focus:border-transparent">
                    </div>
                    ...
                    <div>
                        ...
                        <select name="sppg_id" class="w-full border border-gray-200 rounded-xl px-3 py-2 text-sm focus:ring-2 focus:ring-blue-500 focus:border-transparent">
                            ...
                            @foreach($sppgs as $s)
                            <option value="{{ $s->id }}" {{ old('sppg_id', $relawan->sppg_id) == $s->id ? 'selected' : '' }}>{{ $s->name }}</option>
                            @endforeach
                        </select>
                    </div>

                    <div class="flex items-center gap-3 pt-6">
                        <input type="checkbox" id="aktif" name="aktif" value="1" {{ old('aktif', $relawan->aktif) ? 'checked' : '' }}
