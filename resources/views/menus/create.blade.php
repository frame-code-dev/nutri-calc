                    ...
                    <div class="p-6 grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div>
                            ...
                            <select name="type" id="type" required
                                ...
                                <option value="wet" {{ old('type') === 'wet' ? 'selected' : '' }}>🍱 Menu Basah
                                    (Normal)</option>
                                <option value="dry" {{ old('type') === 'dry' ? 'selected' : '' }}>📦 Menu Kering
                                    (Libur)</option>
                            </select>
                            @error('type')
                                <p class="mt-1.5 text-xs text-red-600 font-medium">{{ $message }}</p>
        ...
        <script>
            ...
            function addIngredientField() {
                ...
                categories.forEach(cat => {
                    ...
                    if (catMaterials.length > 0) {
                        ...
                        catMaterials.forEach(m => {
                            optionsHtml +=
                                `<option value="${m.id}" data-unit="${m.unit}">${m.name} (Base: ${m.unit})</option>`;
                        });
                ...
                commonUnits.forEach(u => {
                    unitOptionsHtml += `<option value="${u}">${u}</option>`;
                });
                ...
                <div class="ingredient-item bg-gray-50/50 p-4 rounded-2xl border border-gray-100 flex flex-col gap-4 animate-in fade-in slide-in-from-bottom-2 duration-300" data-index="${ingredientIndex}">
                    <div class="flex flex-col md:flex-row gap-4 items-start">
                        ...
                        <div class="w-full md:w-40">
                            ...
                            <input type="number" name="items[${ingredientIndex}][quantity_input]" id="qty-${ingredientIndex}"
                                   ...
                                   min="0.001" step="0.001" required placeholder="0" oninput="calculateRow(${ingredientIndex})">
                        </div>
                    ...
                    <!-- Conversion Row (Hidden by default) -->
                        ...
                        <div class="flex items-center gap-3 text-sm text-yellow-800">
                            ...
                            <div class="flex items-center gap-2 flex-1 flex-wrap">
                                ...
                                <input type="number" name="items[${ingredientIndex}][conversion_factor]" id="conv-${ingredientIndex}"
                                   ...
                                   min="0.0001" step="0.0001" value="1" oninput="calculateRow(${ingredientIndex})">
                                <span id="label-unit-base-${ingredientIndex}" class="font-bold">Base</span>
                            </div>
                        </div>
                    </div>
                    
                    <!-- Hidden Calculated Field for Backend Compatibility -->
                    <input type="hidden" name="items[${ingredientIndex}][quantity_per_portion]" id="calc-${ingredientIndex}">
                </div>
            `;

                container.insertAdjacentHTML('beforeend', ingredientHtml);

                // Initialize Tom Select for Material
            ...
            function handleMaterialChange(index, materialId) {
                ...
                // Check if base unit exists in options, if not add it
                let found = false;
